...
notification_count = 0    # 當日發送計數
last_reset_date = ""      # 上次重置計數的日期
...
def main():
    global points, roi_polygon, track_timers, notification_count, last_reset_date
                    ...
                    if is_inside:
                        ...
                        if stay_duration > CONFIG["stay_threshold"] and tid not in notified_ids:
                            ...
                            if vlm_verify_person(frame, box):
                                ...
                                # 準備資訊並繪製在圖片副本上 (不影響即時顯示視窗)
                                alert_frame = frame.copy()
                                start_time_str = datetime.fromtimestamp(track_timers[tid]).strftime("%Y%m%d %H:%M:%S")
                                ...
                                overlay_text = [
                                    ...
                                    f"Location: {CONFIG['stage']}"
                                ]
                                
                                # 在左上角逐行繪製文字
                                for i, line in enumerate(overlay_text):
                                    pos = (20, 40 + i * 35)
                                    # 1. 先繪製較粗的黑色文字作為底色（黑邊），厚度設為 5
                                    cv2.putText(alert_frame, line, pos, 
                                                cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 0), 5, cv2.LINE_AA)
                                    # 2. 再在同一位置繪製黃色(0, 255, 255)文字，厚度設為 2
                                    cv2.putText(alert_frame, line, pos, 
                                ...
                                b64_img = get_base64_image(alert_frame)
                            
                                # 1. 儲存本地告警圖片 (依日期建立子資料夾)
                                daily_dir = os.path.join(CONFIG["alert_dir"], curr_date)
                                if not os.path.exists(daily_dir):
                                    os.makedirs(daily_dir)

                                img_filename = f"alert_ID{tid}_{file_ts}.jpg"
                                img_path = os.path.join(daily_dir, img_filename)
                                cv2.imwrite(img_path, alert_frame)
